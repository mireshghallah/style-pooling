...
def drop_sentence(args):
    ...
    with open(args.input_src, "r") as input_src, open(args.output_src, "w") as output_src, open(args.output_indices, "w") as output_trg:
        for i, ( sent ) in enumerate( (input_src)):
            if (i%args.drop  == 0):                           
                output_src.write(sent)
                output_trg.write(str(i))
                output_trg.write('\n')
...
def main(flags=None):
    ...
    parser.add_argument("--output_indices", type=str, help="subsampled trg file", required=True)
