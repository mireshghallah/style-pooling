...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, help="this is a directory", default="data/blogs_2dom_tmp")
    ...
    parser.add_argument('--max_vocab', type=int, help="this is the number of domains", default=10000)
    ...
    for i in range(args.domains):
        ...
        dev_numbers.append(os.path.join(args.input, "{}.dev.{}.number.{}".format(args.dataset, args.type,i)))
    ...
    ftrain_a = open(os.path.join(save_dir, "dev_3dom.attr"), "w")
    ...
    all_cnt = 0
    for i in range(args.domains):
        with open(devs[i], "r") as fin, open(devs_docs[i], "r") as f_doc_in, open(dev_numbers[i], "r") as age_number:
            for (line, docid, age) in zip(fin, f_doc_in, age_number):
                cnt = 0
                #count UNK
                for word in line.split()[:-1]:
                    if word not in vocab_cut.keys():
