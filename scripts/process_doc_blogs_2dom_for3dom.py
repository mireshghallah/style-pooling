...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, help="this is a directory", default="data/tmp_blogs_2dom/blogs_2dom_distinguished")
    ...
    parser.add_argument('--max_vocab', type=int, help="this is the number of domains", default=22000)
    parser.add_argument('--max_unk', type=int, help="this is the number of domains", default=3)
    ...
    for i in range(args.domains):
        devs_dom_3.append(os.path.join(args.input, "{}.dev.age.{}.trg".format(args.dataset, i)))
        fdevs_dom_3_cleaned.append(open(os.path.join(args.input, "{}.dev_cleaned.3dom.{}.{}".format(args.dataset, args.type,i)),"w+"))
    ...
    ftrain_a_3dom = open(os.path.join(save_dir, "dev.3dom.attr"), "w")
    ...
    all_cnt = 0
    for i in range(args.domains):
        with open(devs[i], "r") as fin, open(devs_docs[i], "r") as f_doc_in, open(devs_dom_3[i], "r") as fdevs_dom_3:
            for (line, docid, age) in zip(fin, f_doc_in, fdevs_dom_3):
                cnt = 0
                #count UNK
                ...
                if cnt < args.max_unk:
                    ...
                    if WRITE:
                        ...
                        ftrain_t.write(line)
                        ftrain_a.write("dom{}\n".format(i))
                        ftrain_a_3dom.write("dom{}\n".format(str(get_age_group(int(age[:-1])))))
